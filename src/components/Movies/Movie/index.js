...
const Movie = ({ movie }) => {
    ...
    return (
        ...
        <Card onMouseLeave={() => setHoverMovieId('')} onMouseOver={() => setHoverMovieId(movie.id)} className="card-image" style={{ flex: 1, marginBottom: "16px", height: "40%" }}>
            ...
            {favorites.some(mov => mov.id === movie.id) ?
                <Button className="button-outline" style={{ position: "absolute", width: "80%", top: "30%", marginLeft: "10%", display: hoverMovieId === movie.id ? "" : "none" }} onClick={() => removeToFavoritesHandler(movie.id)} variant="danger"><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}><FaTrashAlt />Remove from List</div></Button> :
                <Button className="button-outline" style={{ position: "absolute", width: "80%", top: "30%", marginLeft: "10%", display: hoverMovieId === movie.id ? "" : "none" }} onClick={() => addToFavoritesHandler(movie)} variant="success"><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}><FaPlusCircle />Add to List</div></Button>}

            <Button className="button-outline" style={{ position: "absolute", width: "80%", top: "45%", marginLeft: "10%", display: hoverMovieId === movie.id ? "" : "none" }} onClick={() => showDetails(movie.id)} variant="primary"><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}><FaListAlt />Show Details</div></Button>
